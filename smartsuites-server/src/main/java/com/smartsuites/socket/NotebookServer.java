...
import com.smartsuites.notebook.NotebookEventListener;
import com.smartsuites.notebook.NotebookImportDeserializer;
import com.smartsuites.notebook.Paragraph;
import com.smartsuites.notebook.ParagraphJobListener;
import com.smartsuites.notebook.repo.NotebookRepo.Revision;
...
import com.smartsuites.utils.InterpreterBindingUtils;
...
public class NotebookServer extends WebSocketServlet
  ...
  private void sendHomeNote(NotebookSocket conn, HashSet<String> userAndRoles, Notebook notebook,
    ...
    if (note != null) {
      ...
      // 将noteid，websocket加入this.noteMap
      addConnectionToNote(note.getId(), conn);
      conn.send(serializeMessage(new Message(OP.NOTE).put("note", note)));
      // 向前端推送更新
      sendAllAngularObjects(note, user, conn);
  ...
  private void createNote(NotebookSocket conn, HashSet<String> userAndRoles, Notebook notebook,
    ...
    try {
      ...
      if (!StringUtils.isEmpty(defaultInterpreterId)) {
        ...
        for (String interpreterSettingId : notebook.getInterpreterSettingManager().getInterpreterSettingIds()) {
