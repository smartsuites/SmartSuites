...
export default class ScatterchartVisualization extends Nvd3ChartVisualization {
  ...
  constructor (targetElId, config, emitter, jitCompiler, commonService) {
    ...
    this.columnselector = new ColumnselectorTransformation(config, this.columnselectorProps,emitter, jitCompiler)
  }
  ...
  configureChart (chart) {
    ...
    chart.xAxis.tickFormat(function (d) { // TODO remove round after bump to nvd3 > 1.8.5
      return self.xAxisTickFormatWithLabels(Math.round(d * 1e3) / 1e3, self.xLabels)
    })

    chart.yAxis.tickFormat(function (d) { // TODO remove round after bump to nvd3 > 1.8.5
      return self.xAxisTickFormatWithLabels(Math.round(d * 1e3) / 1e3, self.yLabels)
    })
  ...
  xAxisTickFormatWithLabels (d, yLabels) {
  ...
  setScatterChart (data, refresh):any {
